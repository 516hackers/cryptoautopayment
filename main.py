...
AUTO_APPROVE_LOG_PATH = os.path.join(CONFIG_DIR, "auto_approve_log.json")
...
# ── Auto-approve engine ────────────────────────────────────────────────
AUTO_APPROVE_POLL_INTERVAL   = 5    # seconds between pending-list polls
AUTO_APPROVE_BACKOFF_ERRORS  = 3    # consecutive errors before backing off
AUTO_APPROVE_BACKOFF_SECONDS = 30   # how long to back off

# ── TX confirmation ────────────────────────────────────────────────────
TX_WAIT_TIMEOUT      = 180
TX_CONFIRMATIONS_REQ = 2

# ── Security / firewall ────────────────────────────────────────────────
DEFAULT_MAX_TX_AMOUNT        = 5000.0
DEFAULT_DAILY_LIMIT          = 25000.0
DEFAULT_RATE_LIMIT_COUNT     = 10
DEFAULT_RATE_LIMIT_WINDOW    = 60
DEFAULT_HIGH_VALUE_THRESHOLD = 1000.0
SANITY_MAX_AMOUNT            = 1_000_000
MAX_PASSPHRASE_ATTEMPTS      = 3
LOCKOUT_DURATION_SECONDS     = 300
IDLE_LOCK_SECONDS            = 600
ANOMALY_STDEV_MULTIPLIER     = 3.0
...
# ── Colours ────────────────────────────────────────────────────────────
C = {
    "bg":"#F0F4FF","card":"#FFFFFF","hdr_bg":"#1E3A8A","hdr_fg":"#FFFFFF",
    "accent":"#3B82F6","accent2":"#6366F1","success":"#16A34A","warning":"#D97706",
    "danger":"#DC2626","text":"#1E293B","text_dim":"#64748B","border":"#CBD5E1",
    "sim_bg":"#FEF9C3","sim_fg":"#92400E","live_bg":"#FEE2E2","live_fg":"#991B1B",
    "log_bg":"#F8FAFC","row_pend":"#FFFBEB","row_appr":"#DCFCE7","row_rej":"#FEE2E2",
    "row_fail":"#FFE4E6","row_store":"#F3E8FF","auto_bg":"#ECFDF5","auto_fg":"#065F46",
    "auto_off_bg":"#F1F5F9","auto_off_fg":"#64748B",
}
...
NETWORK_PRESETS = {
    "polygon_bh":{
        "label":"Polygon — BH Token  (sends net_amount_bh)","chain_id":137,
...
DEFAULT_CONFIG = {
    ...
    "max_tx_amount":DEFAULT_MAX_TX_AMOUNT,"daily_limit":DEFAULT_DAILY_LIMIT,
    "rate_limit_count":DEFAULT_RATE_LIMIT_COUNT,"rate_limit_window":DEFAULT_RATE_LIMIT_WINDOW,
    "high_value_threshold":DEFAULT_HIGH_VALUE_THRESHOLD,"kill_switch_enabled":False,
}
...
#  Auto-Approve Log
# ══════════════════════════════════════════════════════════════════════
class AutoApproveLog:
    """Crash-safe per-session log of every action taken by the auto-approve
    engine. Written atomically; never deleted on reset."""
    @staticmethod
    def _load() -> list:
        try:
            with open(AUTO_APPROVE_LOG_PATH, encoding="utf-8") as f:
                return json.load(f)
        except Exception:
            return []
    ...
    def _write(data: list):
        ...
        with open(AUTO_APPROVE_LOG_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

    @classmethod
    def record(cls, event: str, withdrawal_id=None, tx_hash: str = "",
                amount: float = 0, wallet: str = "", detail: str = ""):
        data = cls._load()
        data.append({
            "id":           len(data) + 1,
            "event":        event,
...
# ══════════════════════════════════════════════════════════════════════
#  TX History — tamper-evident hash-chained audit log
# ══════════════════════════════════════════════════════════════════════
class TxHistory:
    @staticmethod
    def _load() -> list:
        try:
            with open(TX_HISTORY_PATH, encoding="utf-8") as f: return json.load(f)
    ...
    def record(cls, withdrawal_id, action: str, tx_hash: str = "",
                ...
                wallet: str = "", network: str = "", note: str = "", error: str = ""):
        data  = cls._load()
        prev  = data[-1].get("record_hash", "") if data else ""
        rec   = {
            "id":withdrawal_id and len(data)+1,"withdrawal_id":withdrawal_id,
            "action":action,"tx_hash":tx_hash,"tx_status":tx_status,
            "amount":amount,"wallet":wallet,"network":network,
            "note":note,"error":error,"timestamp":datetime.now().isoformat(),"prev_hash":prev,
        }
        rec["record_hash"] = cls._hash_record(rec, prev)
        data.append(rec); cls._write(data)

    @classmethod
    def all(cls) -> list: return cls._load()
    ...
    def verify_integrity(cls):
        data = cls._load(); prev = ""; legacy = 0
        for rec in data:
            if "record_hash" not in rec: legacy += 1; continue
            stored = rec.get("record_hash","")
            check  = {k:v for k,v in rec.items() if k != "record_hash"}
            if cls._hash_record(check, prev) != stored:
                return False, rec.get("id"), legacy
            prev = stored
...
class DailyLimitTracker:
    ...
    def today_total(cls) -> float:
        return float(cls._load().get(datetime.now().strftime("%Y-%m-%d"), 0.0))

    @classmethod
    def add(cls, amount: float):
        d = cls._load(); key = datetime.now().strftime("%Y-%m-%d")
        d[key] = float(d.get(key, 0.0)) + float(amount)
        cutoff = datetime.now().timestamp() - 30*86400
        for k in list(d.keys()):
            try:
                if datetime.strptime(k, "%Y-%m-%d").timestamp() < cutoff: del d[k]
...
class AddressBlocklist:
    ...
    def remove(cls, addr: str):
        cls._write([a for a in cls._load() if a.get("address","").lower()!=addr.lower()])
...
class LockoutGuard:
    ...
    @classmethod
    def is_locked(cls):
        d = cls._load(); lu = d.get("locked_until")
        if lu and datetime.now().timestamp() < lu: return True, lu
        return False, None

    @classmethod
    def record_failure(cls) -> dict:
        d = cls._load(); d["failed_attempts"] = d.get("failed_attempts",0)+1
        if d["failed_attempts"] >= MAX_PASSPHRASE_ATTEMPTS:
            d["locked_until"] = datetime.now().timestamp()+LOCKOUT_DURATION_SECONDS
            d["failed_attempts"] = 0
        cls._write(d); return d
...
def encrypt_secret(plain: str, pw: str):
    salt = os.urandom(16)
    return base64.b64encode(salt).decode(), Fernet(_derive_key(pw,salt)).encrypt(plain.encode()).decode()

def decrypt_secret(salt_b64: str, token: str, pw: str) -> str:
    return Fernet(_derive_key(pw, base64.b64decode(salt_b64))).decrypt(token.encode()).decode()
...
class ChainClient:
    ...
    def current_block(self) -> int: return self.w3.eth.block_number
    ...
    def wait_for_receipt(self, tx_hash: str, timeout: int = TX_WAIT_TIMEOUT) -> dict:
        deadline = time.time() + timeout
        while time.time() < deadline:
            r = self.get_receipt(tx_hash)
            if r is not None: return r
            time.sleep(3)
        raise ChainError(f"TX not mined within {timeout}s: {tx_hash}")
    ...
    def validate_receipt(self, receipt: dict, tx_hash: str):
        ...
        if status == 0:
            raise TxFailedError(tx_hash,
                "ERC20 transfer reverted (insufficient balance or contract error).")
        raise ChainError(f"Unknown receipt status={status}")

    def preflight_checks(self, from_addr: str, to_addr: str, amount: float,
                          contract_addr: str, decimals: int,
                          required_gas_native: float = 0.002):
        errors = []
        if amount <= 0: errors.append(f"Amount {amount} is zero or negative.")
        units = int(round(amount*(10**decimals)))
        if units <= 0: errors.append(f"Amount rounds to 0 units.")
        try:
            to_cs = self.cs(to_addr)
            if to_cs == self.cs(from_addr):
                errors.append("To == From address.")
        except ChainError as e: errors.append(str(e))
        if errors: raise PreflightError("Pre-send failed:\\n" + "\\n".join(errors))
        try:
            token_bal = self.token_balance(contract_addr, from_addr, decimals)
            if token_bal < amount:
                errors.append(f"Insufficient token: have {token_bal:.6f}, need {amount:.6f}.")
        except Exception as e: errors.append(f"Token balance check failed: {e}")
        try:
            native_bal = self.native_balance(from_addr)
            if native_bal < required_gas_native:
                errors.append(f"Insufficient gas: have {native_bal:.6f}, need {required_gas_native:.4f}.")
        except Exception as e: errors.append(f"Gas balance check failed: {e}")
        if errors: raise PreflightError("Pre-send failed:\\n" + "\\n".join(errors))
    ...
    def send_token(self, private_key: str, from_addr: str, to_addr: str,
        ...
        from_cs = self.cs(from_addr); to_cs = self.cs(to_addr); ccs = self.cs(contract_addr)
        acct = Account.from_key(private_key)
        if acct.address.lower() != from_cs.lower():
            raise ChainError(f"Key address {acct.address} != from {from_cs}")
        units = int(round(amount*(10**decimals)))
        if units <= 0: raise ChainError("Amount rounds to zero.")
        try: gas_price = self.w3.eth.gas_price
        except: gas_price = self.w3.to_wei(30, "gwei")
        contract = self.w3.eth.contract(address=ccs, abi=ERC20_ABI)
        ...
        try:
            est = self.w3.eth.estimate_gas(tx); tx["gas"] = int(est*1.3)
        except Exception: pass
...
class PassphraseDialog(simpledialog.Dialog):
    ...
    def validate(self):
        p = self.e1.get()
        if not p: messagebox.showwarning("Required","Passphrase cannot be empty.",parent=self); return False
...
DETAIL_FIELDS = [
    ("Withdrawal ID","id"),("User ID","user_id"),("Status","status"),
    ("Wallet Address","wallet_address"),("Gross Amount (BH)","gross_amount_bh"),
    ("Platform Fee (BH)","platform_fee_bh"),("Net Amount (BH)","net_amount_bh"),
    ("Net Amount (USD)","net_amount_usd"),("Transaction Hash","transaction_hash"),
    ("Admin Note","admin_note"),("Rejection Reason","rejection_reason"),
    ("Created At","created_at"),("Updated At","updated_at"),("Processed At","processed_at"),
]
...
class AutoApproveEngine:
    """
    Background daemon that polls /pending every AUTO_APPROVE_POLL_INTERVAL
    seconds and approves every new withdrawal automatically.

    Thread safety:
    - _running (threading.Event) controls the loop
    - _stop_event signals the sleep to wake up early
    - All UI updates go through app.ui() (root.after) — never direct calls
    - All file I/O uses the same JSON helpers as the rest of the app
    - All 8 security layers are enforced identically to the manual path
    """
    ...
    def __init__(self, app: "App"):
        ...
        # session state (reset each start)
        self._approved_ids:  set  = set()   # IDs approved this session (no double-approval)
        self._skipped_ids:   set  = set()   # IDs permanently skipped this session (blocked/bad)
        ...
        self._rate_ts:       list = []      # sliding window for rate limiting

    @property
    def is_running(self) -> bool:
        return self._running

    def start(self, pk: str, cfg: dict, api: ApiClient):
        """Start the engine. Must be called from the UI thread."""
        with self._lock:
            if self._running: return
            self._pk  = pk
            self._cfg = dict(cfg)      # snapshot so mid-run config changes don't cause races
            self._api = api
            self._running    = True
            ...
            self._rate_ts       = []
        self._thread = threading.Thread(target=self._loop, daemon=True, name="AutoApprove")
        self._thread.start()
        AutoApproveLog.record("engine_start")
        SecurityLog.record("auto_approve_start", f"network={cfg.get('network')}")

    def stop(self):
        """Signal the engine to stop. Returns immediately; thread winds down within ~5 s."""
        with self._lock:
            if not self._running: return
            self._running = False
            self._stop_event.set()
        AutoApproveLog.record("engine_stop",
            detail=f"approved={self._total_success} failed={self._total_fail}")
        SecurityLog.record("auto_approve_stop",
            f"approved={self._total_success} failed={self._total_fail} total={fmt(self._session_total)}")

    def status_dict(self) -> dict:
        return {
            "running":       self._running,
            "approved":      self._total_success,
    ...
    # ── Main loop ─────────────────────────────────────────────────────
    def _loop(self):
        self.app.log("🤖 Auto-Approve Engine STARTED. Polling every "
                     f"{AUTO_APPROVE_POLL_INTERVAL}s…", tag="ok")
        while self._running:
            try:
                self._poll_and_approve()
                self._consec_errors = 0
            except Exception as exc:
                self._consec_errors += 1
                err_msg = str(exc)
                self.app.log(f"🤖 AUTO-ENGINE ERROR #{self._consec_errors}: {err_msg}", tag="fail")
                AutoApproveLog.record("engine_error", detail=err_msg)
                if self._consec_errors >= AUTO_APPROVE_BACKOFF_ERRORS:
                    self.app.log(f"🤖 {AUTO_APPROVE_BACKOFF_ERRORS} consecutive errors — "
                                 f"backing off for {AUTO_APPROVE_BACKOFF_SECONDS}s…", tag="warn")
                    self._stop_event.wait(timeout=AUTO_APPROVE_BACKOFF_SECONDS)
                    self._consec_errors = 0
                    if not self._running: break
                    continue

            self._last_poll = datetime.now().strftime("%H:%M:%S")
            self.app.ui(self.app._update_auto_status)

            # Interruptible sleep
            self._stop_event.wait(timeout=AUTO_APPROVE_POLL_INTERVAL)
            if self._stop_event.is_set(): break

        self._running = False
        self.app.log("🤖 Auto-Approve Engine STOPPED.", tag="warn")
        self.app.ui(self.app._update_auto_status)
        self.app.ui(lambda: self.app.refresh_pending(silent=True))
        self.app.ui(lambda: self.app.refresh_all(silent=True))

    def _poll_and_approve(self):
        cfg          = self._cfg
        amount_field = "net_amount_bh" if cfg["amount_source"] == "bh" else "net_amount_usd"
        simulate     = cfg.get("simulate_only", True)
        preset       = NETWORK_PRESETS.get(cfg["network"], {})
        chain_id     = preset.get("chain_id", 137)
        explorer     = preset.get("explorer_tx", "")

        # Kill-switch check (re-read config from disk each poll for real-time effect)
        live_cfg = ConfigStore.load()
        if not simulate and live_cfg.get("kill_switch_enabled"):
            self.app.log("🤖 Kill switch active — auto-engine skipping this poll.", tag="warn")
            return

        # Fetch pending list
        try:
            records = self._api.list_pending()
        except ApiError as e:
            raise RuntimeError(f"API list_pending failed: {e}")

        new_records = [r for r in records
                       if str(r.get("id")) not in self._approved_ids
                       and str(r.get("id")) not in self._skipped_ids]

        if not new_records:
            return  # nothing to do this poll

        self.app.log(f"🤖 Auto-engine: {len(new_records)} new pending withdrawal(s) found.", tag="ok")

        # Build a chain client once per poll (reuse across withdrawals in this batch)
        chain = None
        nonce = None
        if not simulate:
            rpcs = [cfg["rpc_url"]]
            rpcs += (SCAN_NETWORKS["polygon"] if "polygon" in cfg["network"]
                     else SCAN_NETWORKS["bsc"])["rpcs"]
            chain = ChainClient.from_rpcs(list(dict.fromkeys(rpcs)))
        ...
        for rec in new_records:
            ...
            # ── FIREWALL LAYER 1: format / sanity ────────────────────
            if not math.isfinite(amount) or amount <= 0:
                msg = f"Amount {amount} invalid."
                self._skip(rid, rec, msg, amount, to_addr, "approve_preflight_fail", cfg)
                continue

            if amount > SANITY_MAX_AMOUNT:
                msg = f"Amount {amount} exceeds sanity ceiling {SANITY_MAX_AMOUNT}."
                self._skip(rid, rec, msg, amount, to_addr, "approve_blocked_sanity", cfg)
                SecurityLog.record("sanity_ceiling_block_auto", f"#{rid} amount={amount}")
                continue

            if not is_valid_address(to_addr):
                msg = f"Invalid address format: {to_addr!r}"
                self._skip(rid, rec, msg, amount, to_addr, "approve_preflight_fail", cfg)
                continue

            # ── FIREWALL LAYER 1B: blocklist ─────────────────────────
            if AddressBlocklist.is_blocked(to_addr):
                msg = "Address is on the security blocklist."
                self._skip(rid, rec, msg, amount, to_addr, "approve_blocked_address", cfg)
                SecurityLog.record("blocked_address_auto", f"#{rid} -> {to_addr}")
                continue

            if not simulate:
                # ── FIREWALL LAYER 1C: per-tx cap ────────────────────
                max_tx = float(live_cfg.get("max_tx_amount") or 0)
                if max_tx > 0 and amount > max_tx:
                    msg = f"Amount {fmt(amount)} > per-tx cap {fmt(max_tx)}."
                    self._skip(rid, rec, msg, amount, to_addr, "approve_blocked_cap", cfg)
                    SecurityLog.record("tx_cap_block_auto", f"#{rid} {msg}")
                    continue

                # ── FIREWALL LAYER 1D: daily cap ─────────────────────
                daily_limit = float(live_cfg.get("daily_limit") or 0)
                if daily_limit > 0:
                    used = DailyLimitTracker.today_total() + self._session_total
                    if used + amount > daily_limit:
                        msg = f"Daily limit reached ({fmt(used)} + {fmt(amount)} > {fmt(daily_limit)})."
                        self.app.log(f"🤖 #{rid}: ⛔ {msg}", tag="fail")
                        AutoApproveLog.record("daily_limit_reached", withdrawal_id=rid, detail=msg)
                        SecurityLog.record("daily_limit_block_auto", msg)
                        self.app.log("🤖 Daily limit reached — auto-engine pausing until tomorrow or limit reset.", tag="warn")
                        self.stop()
                        return

                # ── FIREWALL LAYER 1E: rate limit ────────────────────
                rate_n = int(live_cfg.get("rate_limit_count") or 0)
                rate_w = float(live_cfg.get("rate_limit_window") or DEFAULT_RATE_LIMIT_WINDOW)
                now_ts = time.time()
                self._rate_ts = [t for t in self._rate_ts if now_ts - t < rate_w]
                if rate_n > 0 and len(self._rate_ts) >= rate_n:
                    wait = rate_w - (now_ts - self._rate_ts[0]) + 1
                    self.app.log(f"🤖 Rate limit hit — waiting {wait:.0f}s…", tag="warn")
                    self._stop_event.wait(timeout=max(wait, 1))
                    if not self._running: return
                    self._rate_ts = [t for t in self._rate_ts if time.time()-t < rate_w]

            try:
                stored = PendingTxStore.get(rid)
                if stored:
                    tx_hash = stored["tx_hash"]
                    self.app.log(f"🤖 #{rid}: ♻ Stored TX found — retrying API only.", tag="stored")
                    if not simulate and chain:
                        try:
                            ...
                            self.app.log(f"🤖 #{rid}: ❌ Stored TX failed on-chain — skipping.", tag="fail")
                            TxHistory.record(rid, "approve_failed_tx", tx_hash=tx_hash,
                                tx_status="reverted", amount=amount, wallet=to_addr,
                                network=cfg["network"], error=str(tf))
                            PendingTxStore.remove(rid)
                            self._total_fail += 1; self._skipped_ids.add(rid); continue
                        except ChainError: pass  # timeout — still try api

                elif simulate:
                    tx_hash = f"AUTOSIM-{rid}-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
                    self.app.log(f"🤖 #{rid}: [SIM] would send {fmt(amount)} → {to_addr}", tag="sim")

                else:
                    # ── LAYER 2: pre-flight ───────────────────────────
                    chain.preflight_checks(cfg["from_address"], to_addr, amount,
                                           cfg["token_contract"], cfg["decimals"],
                                           required_gas_native=0.002)

                    # ── LAYER 3: send ─────────────────────────────────
                    self.app.log(f"🤖 #{rid}: 📡 Sending {fmt(amount)} → {to_addr[:20]}…", tag="ok")
                    tx_hash = chain.send_token(self._pk, cfg["from_address"], to_addr,
                                               amount, cfg["token_contract"],
                                               cfg["decimals"], chain_id, nonce)
                    nonce += 1

                    # ── LAYER 4: crash-safe store ─────────────────────
                    PendingTxStore.put(rid, tx_hash, to_addr, amount, cfg["network"])
                    url = explorer + tx_hash if explorer else ""
                    self.app.log(f"🤖 #{rid}: 📤 Broadcast: {tx_hash[:22]}…", url=url, tag="ok")

                    # ── LAYER 5: wait for receipt ─────────────────────
                    try:
                        receipt = chain.wait_for_receipt(tx_hash, timeout=TX_WAIT_TIMEOUT)
                    except ChainError as ce:
                        self.app.log(f"🤖 #{rid}: ⚠ Receipt timeout — TX in store. Manual check needed.", tag="warn")
                        TxHistory.record(rid, "approve_timeout", tx_hash=tx_hash,
                            tx_status="timeout", amount=amount, wallet=to_addr,
                            network=cfg["network"], error=str(ce))
                        self._total_fail += 1; self._skipped_ids.add(rid); continue

                    # ── LAYER 6: CRITICAL receipt status check ─────────
                    try:
                        chain.validate_receipt(receipt, tx_hash)
                        confs = chain.current_block() - receipt.get("blockNumber", 0)
                        self.app.log(f"🤖 #{rid}: ✅ TX SUCCESS on-chain ({confs} confs).", tag="ok")
                    except TxFailedError as tf:
                        self.app.log(
                            f"🤖 #{rid}: ❌❌ TX FAILED ON-CHAIN — NOT approving. "
                            f"Reason: {tf.reason}", tag="fail")
                        TxHistory.record(rid, "approve_failed_tx", tx_hash=tx_hash,
                            tx_status="reverted", amount=amount, wallet=to_addr,
                            network=cfg["network"], error=str(tf))
                        AutoApproveLog.record("tx_reverted", rid, tx_hash, amount, to_addr, str(tf))
                        self._total_fail += 1; self._skipped_ids.add(rid); continue

                    # ── LAYER 7: confirmation count ───────────────────
                    if confs < TX_CONFIRMATIONS_REQ:
                        self._stop_event.wait(timeout=6)

                # ── LAYER 8: API approve ──────────────────────────────
                tx_status = "simulated" if simulate else "success"
                self._api.approve(rid, tx_hash,
                    note=(f"AUTO|{'SIMULATED' if simulate else cfg['network']}|"
                          f"{amount_field}={fmt(amount)}|{tx_status}"))

                PendingTxStore.remove(rid)
                TxHistory.record(rid, "approve", tx_hash=tx_hash, tx_status=tx_status,
                    amount=amount, wallet=to_addr, network=cfg["network"],
                    note=f"auto_approve|{'simulated' if simulate else 'confirmed'}")
                if not simulate:
                    DailyLimitTracker.add(amount)
                    self._rate_ts.append(time.time())
                    self._session_total += amount

                url = (explorer + tx_hash) if (explorer and not simulate) else ""
                self.app.log(f"🤖 #{rid}: ✅ AUTO-APPROVED {fmt(amount)} → {to_addr[:20]}…",
                             url=url, tag="ok")
                AutoApproveLog.record("approved", rid, tx_hash, amount, to_addr,
                                      f"{'simulated' if simulate else 'confirmed'}")
                self._approved_ids.add(rid)
                self._total_success += 1
                self._last_action = f"#{rid} approved {fmt(amount)}"
                self.app.ui(self.app._update_auto_status)

            except PreflightError as pe:
                self.app.log(f"🤖 #{rid}: ⛔ Pre-flight failed: {pe}", tag="preflight")
                TxHistory.record(rid, "approve_preflight_fail", tx_status="skipped",
                    amount=amount, wallet=to_addr, network=cfg["network"], error=str(pe))
                AutoApproveLog.record("preflight_fail", rid, detail=str(pe))
                self._total_fail += 1; self._skipped_ids.add(rid)

            except ApiError as ae:
                self.app.log(f"🤖 #{rid}: ⚠ TX sent but API approve FAILED: {ae}", tag="warn")
                TxHistory.record(rid, "approve_api_fail", tx_hash=tx_hash if "tx_hash" in dir() else "",
                    tx_status="success_tx_api_fail", amount=amount, wallet=to_addr,
                    network=cfg["network"], error=str(ae))
                # Keep in PendingTxStore for retry; DON'T add to approved_ids or skipped_ids
                # so the engine will retry next poll

            except Exception as exc:
                tb = traceback.format_exc()
                self.app.log(f"🤖 #{rid}: ❌ UNEXPECTED ERROR: {exc}", tag="fail")
                TxHistory.record(rid, "approve_error", tx_status="error",
                    amount=amount, wallet=to_addr, network=cfg["network"], error=str(exc))
                AutoApproveLog.record("unexpected_error", rid, detail=str(exc))
                self._total_fail += 1
                raise  # re-raise so the outer loop counts consecutive errors

    def _skip(self, rid, rec, msg, amount, wallet, action_type, cfg):
        self.app.log(f"🤖 #{rid}: ⛔ {msg}", tag="fail")
        TxHistory.record(rid, action_type, tx_status="skipped",
            amount=amount, wallet=wallet, network=cfg["network"], error=msg)
        AutoApproveLog.record(action_type, rid, detail=msg)
        self._total_fail += 1
        self._skipped_ids.add(rid)
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        # Auto-approve engine
        self.auto_engine = AutoApproveEngine(self)
        ...
        for name in ("imh.png","ayamil.jpg"):
            ...
            if os.path.exists(p):
                try:
                    ...
                    self._favicon_ref = img; return
                except Exception: continue

    # ── Style (clam forced for visible tab text) ──────────────────────
    def _apply_style(self):
        ...
        s.configure("Danger.TButton",  background=C["danger"],  foreground="#fff",
                    font=("Segoe UI",9,"bold"), padding=[12,6])
        s.configure("Auto.TButton",    background="#059669",    foreground="#fff",
    ...
    # ── Live tickers ──────────────────────────────────────────────────
    def _interval(self, key, live_i, bg_i):
        return live_i if self._active_tab_key == key else bg_i

    def _tick_all(self):
        if not self._live_paused:
            iv = self._interval("all", LIVE_INTERVAL_ALL, BACKGROUND_INTERVAL_ALL)
            if self._all_countdown <= 0:
                self.refresh_all(silent=True); self._all_countdown = iv
            else: self._all_countdown -= 1
            if hasattr(self,"all_next_lbl"):
                live = self._active_tab_key == "all"
                self.all_next_lbl.config(text=f"{'🟢 LIVE' if live else '⏸ bg'} • {self._all_countdown}s")
        self.root.after(1000, self._tick_all)

    def _tick_pending(self):
        if not self._live_paused:
            iv = self._interval("pending", LIVE_INTERVAL_PENDING, BACKGROUND_INTERVAL_PENDING)
            if self._pend_countdown <= 0:
                self.refresh_pending(silent=True); self._pend_countdown = iv
            else: self._pend_countdown -= 1
            if hasattr(self,"pend_next_lbl"):
                live = self._active_tab_key == "pending"
                self.pend_next_lbl.config(text=f"{'🟢 LIVE' if live else '⏸ bg'} • {self._pend_countdown}s")
        self.root.after(1000, self._tick_pending)

    def _tick_balances(self):
        if not self._live_paused:
            iv = self._interval("balances", LIVE_INTERVAL_BALANCES, BACKGROUND_INTERVAL_BALANCES)
            if self._bal_countdown <= 0:
                if self.cfg.get("from_address"): self.refresh_balances(silent=True)
                self._bal_countdown = iv
            else: self._bal_countdown -= 1
            if hasattr(self,"bal_next_lbl"):
                live = self._active_tab_key == "balances"
                self.bal_next_lbl.config(text=f"{'🟢 LIVE' if live else '⏸ bg'} • {self._bal_countdown}s")
        self.root.after(1000, self._tick_balances)

    def _tick_history(self):
        if not self._live_paused:
            iv = self._interval("history", LIVE_INTERVAL_HISTORY, BACKGROUND_INTERVAL_HISTORY)
            if self._hist_countdown <= 0:
                self.refresh_history(); self._hist_countdown = iv
            else: self._hist_countdown -= 1
            if hasattr(self,"hist_next_lbl"):
                live = self._active_tab_key == "history"
                self.hist_next_lbl.config(text=f"{'🟢 LIVE' if live else '⏸ bg'} • {self._hist_countdown}s")
    ...
    def _tick_security(self):
        if self.runtime_pk and (time.time()-self._last_activity) > IDLE_LOCK_SECONDS:
            self.runtime_pk = None
            SecurityLog.record("idle_lock","Key cleared after idle.")
            self.log("🔒 Wallet key auto-locked after idle timeout.", tag="warn")
            if self.auto_engine.is_running:
                self.auto_engine.stop()
                self.log("🤖 Auto-approve engine stopped (wallet key was locked).", tag="warn")
        if hasattr(self,"_refresh_security_banner"): self._refresh_security_banner()
    ...
    def _on_tab_changed(self, event=None):
        ...
        self._mark_activity()
        snap = {"all":0,"pending":1,"balances":2,"history":4}
        for k,attr in (("all","_all_countdown"),("pending","_pend_countdown"),
                        ("balances","_bal_countdown"),("history","_hist_countdown")):
            if self._active_tab_key == k: setattr(self, attr, 0)
        if self._active_tab_key == "security":
            self._reload_security_log(); self._reload_block_tree()
    ...
    def _get_pk(self):
        ...
        if locked:
            ...
            raise ChainError(f"🔒 Wallet locked. Try again in {wait}s.")
        if self.cfg.get("pk_set"):
            ...
            try:
                ...
                SecurityLog.record("passphrase_fail")
                if d.get("locked_until"):
                    SecurityLog.record("passphrase_lockout")
                    raise ChainError(f"🔒 Too many failed attempts. Locked for {LOCKOUT_DURATION_SECONDS//60}min.")
                rem = MAX_PASSPHRASE_ATTEMPTS - d.get("failed_attempts",0)
                raise ChainError(f"Wrong passphrase. {max(rem,0)} attempt(s) before lockout.")
            LockoutGuard.record_success()
            self.runtime_pk = pk; self._last_activity = time.time(); return pk
    ...
    # ══════════════════════════════════════════════════════════════════
    def _build_ui(self):
        hdr = tk.Frame(self.root, bg=C["hdr_bg"], pady=10); hdr.pack(fill="x")
        lf = tk.Frame(hdr, bg=C["hdr_bg"]); lf.pack(side="left", padx=16)
        tk.Label(lf,text="∞",font=("Segoe UI",22,"bold"),bg=C["hdr_bg"],fg="#60A5FA").pack(side="left")
        tf = tk.Frame(lf, bg=C["hdr_bg"]); tf.pack(side="left", padx=6)
        tk.Label(tf,text=APP_TITLE,font=("Segoe UI",13,"bold"),bg=C["hdr_bg"],fg="#FFFFFF").pack(anchor="w")
        tk.Label(tf,text=APP_CREDIT,font=("Segoe UI",8),bg=C["hdr_bg"],fg="#93C5FD").pack(anchor="w")
        hdr_r = tk.Frame(hdr, bg=C["hdr_bg"]); hdr_r.pack(side="right", padx=16)
        self.mode_badge = tk.Label(hdr_r,text="",font=("Segoe UI",9,"bold"),padx=10,pady=4)
        ...
        # Auto-approve quick-status in header
        self.auto_hdr_lbl = tk.Label(hdr_r,text="🤖 AUTO: OFF",font=("Segoe UI",8,"bold"),
                                      bg=C["auto_off_bg"],fg=C["auto_off_fg"],padx=8,pady=4)
        self.auto_hdr_lbl.pack(side="right",padx=4)
        ...
        sbar = tk.Frame(self.root, bg=C["hdr_bg"], pady=3); sbar.pack(fill="x",side="bottom")
        tk.Label(sbar,text=f"  {APP_CREDIT}  •  {APP_TITLE} v{APP_VERSION}",
                 bg=C["hdr_bg"],fg="#93C5FD",font=("Segoe UI",8)).pack(side="left")
        self.status_clock = tk.Label(sbar,text="",bg=C["hdr_bg"],fg="#E2E8F0",
    ...
    def _row_values(rec):
        return (rec.get("id"),rec.get("user_id"),
                ...
                rec.get("wallet_address"),rec.get("status"),(rec.get("created_at") or "")[:19])

    def _populate_tree(self, tree, records):
        for row in tree.get_children(): tree.delete(row)
        for rec in records:
            tree.insert("","end",iid=str(rec.get("id")),
                        values=self._row_values(rec),tags=(rec.get("status","pending"),))
    ...
    def _update_auto_status(self):
        """Called from UI thread to refresh all auto-approve display widgets."""
        st = self.auto_engine.status_dict()
        if st["running"]:
            self.auto_hdr_lbl.config(
                text=f"🤖 AUTO: ON ✅{st['approved']}",
                bg=C["auto_bg"],fg=C["auto_fg"])
            if hasattr(self,"auto_start_btn"):
                self.auto_start_btn.config(state="disabled")
                self.auto_stop_btn.config(state="normal")
            if hasattr(self,"auto_stat_lbl"):
                self.auto_stat_lbl.config(
                    text=f"✅ {st['approved']} approved   ❌ {st['failed']} failed   "
                         f"💰 {fmt(st['session_total'])} total   "
                         f"Last: {st['last_action']}   "
                         f"Poll: {st['last_poll']}",
                    fg=C["auto_fg"],bg=C["auto_bg"])
        else:
            self.auto_hdr_lbl.config(text="🤖 AUTO: OFF",bg=C["auto_off_bg"],fg=C["auto_off_fg"])
            if hasattr(self,"auto_start_btn"):
                self.auto_start_btn.config(state="normal")
                self.auto_stop_btn.config(state="disabled")
            if hasattr(self,"auto_stat_lbl"):
                self.auto_stat_lbl.config(
                    text=(f"Last session: {st['approved']} approved / {st['failed']} failed / "
                          f"{fmt(st['session_total'])} total"
                          if st["approved"]+st["failed"] > 0 else "Not started this session."),
                    fg=C["text_dim"],bg=C["auto_off_bg"])
    ...
    def _record_popup(self, rec):
        ...
        if stored:
            ...
            tk.Label(w,text="⚠ ON-CHAIN TX STORED",font=("Segoe UI",9,"bold"),
                     bg="#FEF3C7",fg="#92400E").pack(anchor="w")
            tk.Label(w,text=f"TX: {stored['tx_hash']}",font=("Consolas",8),
        ...
        for i,(label,key) in enumerate(DETAIL_FIELDS):
            val = rec.get(key); val_str = "—" if val is None else str(val)
            ...
            if key in ("wallet_address","transaction_hash"): vf=("Consolas",8); vc="#1E40AF"
        ...
        if hist:
            ...
            tk.Label(body,text=f"TX History ({len(hist)} actions)",
                     font=("Segoe UI",9,"bold"),bg=C["bg"],fg=C["hdr_bg"],padx=14).pack(anchor="w")
            for h in reversed(hist):
                ...
                tk.Label(hrow,
                         text=f"[{(h.get('timestamp') or '')[:19]}]  {h.get('action','').upper()}  "
                              f"status={h.get('tx_status','?')}  tx={str(h.get('tx_hash',''))[:20]}…",
        ...
        if rec.get("wallet_address"):
            ...
            ttk.Button(btn_row,text="🔗 Explorer",
                       command=lambda: safe_open_url(f"{exp}/address/{rec['wallet_address']}")).pack(side="right",padx=4)
    ...
    def _build_all_tab(self):
        ...
        for i,(key,label,col) in enumerate([
            ...
            box=tk.Frame(sc,bg=C["bg"],padx=12,pady=6,highlightbackground=col,highlightthickness=2)
            box.grid(row=0,column=i,padx=4,pady=2,sticky="nsew"); sc.columnconfigure(i,weight=1)
        ...
        self.all_next_lbl=tk.Label(bar,text="",font=("Segoe UI",8),bg=C["bg"],fg=C["text_dim"])
    ...
    def refresh_all(self, silent=False):
        self._set_api()
        self._all_countdown = self._interval("all",LIVE_INTERVAL_ALL,BACKGROUND_INTERVAL_ALL)
        sv = getattr(self,"var_all_status",None); status = sv.get() if sv else "all"
        def work():
            ...
            return records, st
        def done(res):
            records, st = res; self.all_records = records
    ...
    # ══════════════════════════════════════════════════════════════════
    #  TAB 2 — Pending + Auto-Approve Dashboard
    # ══════════════════════════════════════════════════════════════════
    def _build_pending_tab(self):
        p = self.tab_pending

        # ── Auto-Approve Dashboard card ────────────────────────────────
        auto_card = tk.Frame(p, bg=C["auto_off_bg"], highlightbackground=C["border"],
                             highlightthickness=2); auto_card.pack(fill="x",padx=10,pady=(8,4))
        auto_hdr = tk.Frame(auto_card,bg=C["hdr_bg"],pady=6); auto_hdr.pack(fill="x")
        tk.Label(auto_hdr,text="  🤖 Auto-Approve Engine",font=("Segoe UI",10,"bold"),
                 bg=C["hdr_bg"],fg="#fff").pack(side="left")
        tk.Label(auto_hdr,text=f"  polls every {AUTO_APPROVE_POLL_INTERVAL}s  •  all security layers active  ",
                 font=("Segoe UI",8),bg=C["hdr_bg"],fg="#93C5FD").pack(side="right")

        # Controls row
        ctrl = tk.Frame(auto_card,bg=C["auto_off_bg"],padx=12,pady=8); ctrl.pack(fill="x")
        self.auto_start_btn = ttk.Button(ctrl,text="▶  START AUTO-APPROVE",
                                          style="Auto.TButton",command=self._start_auto_approve)
        self.auto_start_btn.pack(side="left",padx=4)
        self.auto_stop_btn  = ttk.Button(ctrl,text="⏹  STOP",style="Danger.TButton",
                                          command=self._stop_auto_approve,state="disabled")
        self.auto_stop_btn.pack(side="left",padx=4)

        # Status line
        self.auto_stat_lbl = tk.Label(auto_card,text="Not started this session.",
                                       font=("Segoe UI",9),bg=C["auto_off_bg"],
                                       fg=C["text_dim"],padx=14,pady=6,anchor="w")
        self.auto_stat_lbl.pack(fill="x")

        # Warnings
        warn_frame = tk.Frame(auto_card,bg="#FEF3C7",padx=12,pady=6)
        warn_frame.pack(fill="x")
        tk.Label(warn_frame,
                 text=("⚠  Auto-approve runs all 8 security/firewall layers on every withdrawal. "
                       "It will NOT approve a withdrawal if: kill switch is on, address is blocked, "
                       "daily cap exceeded, per-tx cap exceeded, rate limit hit, pre-flight balance "
                       "fails, OR the on-chain tx receipt status is 0 (reverted). "
                       "Activity is logged to tx_history.json and auto_approve_log.json."),
                 font=("Segoe UI",8),bg="#FEF3C7",fg="#92400E",
                 wraplength=1200,justify="left").pack(anchor="w")

        # ── Summary row ────────────────────────────────────────────────
        sc=tk.Frame(p,bg=C["card"],highlightbackground=C["border"],highlightthickness=1)
        sc.pack(fill="x",padx=10,pady=(4,2))
        ...
        self.pending_mode_lbl=tk.Label(si,text="",font=("Segoe UI",9,"bold"),padx=10,pady=4)
        ...
        self.pend_next_lbl=tk.Label(bar,text="",font=("Segoe UI",8),bg=C["bg"],fg=C["text_dim"])
        ...
        tk.Button(lh,text="Clear",font=("Segoe UI",8),bg=C["hdr_bg"],fg="#93C5FD",
                  relief="flat",bd=0,command=_clear_log).pack(side="right",padx=8)
        lb=tk.Frame(lc,bg=C["log_bg"]); lb.pack(fill="x")
        self.log_text=tk.Text(lb,height=9,wrap="word",state="disabled",
                               font=("Consolas",9),bg=C["log_bg"],fg=C["text"],relief="flat",bd=0)
        ...
        for tag,fg,font in [("link",C["accent"],None),("ok",C["success"],None),
                             ("fail",C["danger"],None),("warn",C["warning"],None),
                             ("sim","#92400E",None),("stored",C["accent2"],("Consolas",9,"bold")),
                             ("preflight","#7C2D12",("Consolas",9,"bold"))]:
            kw={"foreground":fg}
            if tag=="link": kw["underline"]=1
            if font: kw["font"]=font
            self.log_text.tag_configure(tag,**kw)

    # ── Auto-approve start/stop ───────────────────────────────────────
    def _start_auto_approve(self):
        # Must be in LIVE mode to auto-approve real funds
        if not self.var_simulate.get():
            if self.cfg.get("kill_switch_enabled"):
                messagebox.showerror("Kill Switch Active",
                    "The kill switch is ON. Deactivate it in the 🛡 Security tab first."); return

            if not messagebox.askyesno("Start Auto-Approve (LIVE)",
                "⚠ LIVE MODE — real on-chain transactions will be sent automatically.\n\n"
                "The engine will:\n"
                "  • Poll for new pending withdrawals every 5 seconds\n"
                "  • Approve each one automatically, 24/7\n"
                "  • Enforce ALL security layers (kill switch, caps, blocklist, receipt check)\n"
                "  • Log every action to tx_history.json and auto_approve_log.json\n\n"
                "Start?", icon="warning"): return

            if not messagebox.askyesno("Second Confirmation",
                "Confirm: The engine will send REAL funds automatically without further prompts.\n\n"
                "YES — I understand, start the auto-approve engine.", icon="warning"): return

        try:
            pk = self._get_pk() if not self.var_simulate.get() else "__SIMULATE__"
        except ChainError as e:
            messagebox.showerror("Key required", str(e)); return

        cfg_snap = dict(self.cfg)
        cfg_snap["simulate_only"] = self.var_simulate.get()
        self.auto_engine.start(pk, cfg_snap,
                                ApiClient(self.cfg["api_base_url"],self.cfg["auth_header"]))
        self._update_auto_status()
        self.log(f"🤖 AUTO-APPROVE STARTED ({'SIMULATION' if self.var_simulate.get() else 'LIVE'})", tag="ok")

    def _stop_auto_approve(self):
        self.auto_engine.stop()
        self._update_auto_status()
    ...
    def _show_pending_store(self):
        ...
        win=tk.Toplevel(self.root); win.title("Pending TX Store"); win.geometry("720x380"); win.grab_set()
        ...
        if not store: t.insert("end","✓ No pending transactions.\\n")
        else:
    ...
    def refresh_pending(self, silent=False):
        self._set_api()
        self._pend_countdown = self._interval("pending",LIVE_INTERVAL_PENDING,BACKGROUND_INTERVAL_PENDING)
    ...
    def reject_selected(self):
        ...
        if not note: messagebox.showwarning("Cancelled","Reason required."); return
        self._set_api()
        def work():
            for r in recs:
                try:
                    ...
                    self.log(f"#{r['id']}: Rejected.",tag="warn")
                except ApiError as e:
                    TxHistory.record(r["id"],"reject",tx_status="n/a",error=str(e))
                    self.log(f"#{r['id']}: Reject FAILED — {e}",tag="fail")
        self.run_bg(work,on_done=lambda _: (self.refresh_pending(),self.refresh_all(),self.refresh_history()))
    ...
    def approve_all(self):
        if not self.pending_records: messagebox.showinfo("Nothing to do","No pending."); return
        self._approve_batch(list(self.pending_records))

    # ── Manual approve batch (same 8 layers as engine) ─────────────────
    def _approve_batch(self, recs: list):
        cfg=self.cfg; amount_field="net_amount_bh" if cfg["amount_source"]=="bh" else "net_amount_usd"
        simulate=self.var_simulate.get(); mode_str="SIMULATION" if simulate else "⚠ LIVE"
        if not simulate and cfg.get("kill_switch_enabled"):
            messagebox.showerror("Kill Switch Active","All live approvals blocked."); return
        if not cfg["from_address"]: messagebox.showwarning("Missing","Set From Address."); return
        total=sum(float(r.get(amount_field,0) or 0) for r in recs)
        stored_ids=[str(r["id"]) for r in recs if PendingTxStore.get(r["id"])]
        stored_msg=f"\\n\\n⚠ {len(stored_ids)} stored TX(s) — chain send skipped, API retried." if stored_ids else ""
        if not messagebox.askyesno("Confirm",f"Mode: {mode_str}\\nApprove {len(recs)}?\\n"
                                    f"Total: {fmt(total)}\\nFrom: {cfg['from_address']}{stored_msg}"): return
        ...
        if not simulate and threshold>0 and total>threshold:
            typed=simpledialog.askstring("High-Value Confirm",
                f"Total {fmt(total)} > threshold {fmt(threshold)}.\\nType APPROVE to continue:",parent=self.root)
            if (typed or "").strip().upper()!="APPROVE":
                messagebox.showwarning("Cancelled","Cancelled."); return
        pk=None
        if not simulate:
            try: pk=self._get_pk()
            except ChainError as e: messagebox.showerror("Key required",str(e)); return
        self._set_api()
        preset=NETWORK_PRESETS.get(cfg["network"],{}); chain_id=preset.get("chain_id",137)
        explorer=preset.get("explorer_tx","")
        self.log(f"Batch start: {len(recs)} | {'SIMULATE' if simulate else 'LIVE'}",tag="sim" if simulate else "fail")

        def work():
            if not simulate and cfg.get("kill_switch_enabled"):
                self.log("🛑 ABORT: Kill switch.",tag="fail"); return 0,len(recs)
            chain=None; nonce=None
            if not simulate:
                try: chain=self._new_chain(); nonce=chain.next_nonce(cfg["from_address"])
                except ChainError as e: self.log(f"ABORT: {e}",tag="fail"); return 0,len(recs)
            ok_cnt=0; fail_cnt=0; live_cfg=ConfigStore.load()
            batch_total=0.0; rate_ts=list(self._approval_timestamps)
            for rec in recs:
                rid=str(rec.get("id")); to_addr=(rec.get("wallet_address") or "").strip()
                try: amount=float(rec.get(amount_field,0) or 0)
                except: amount=0
                # LAYER 1
                if not math.isfinite(amount) or amount<=0:
                    self.log(f"#{rid}: ⛔ Invalid amount {amount}",tag="fail")
                    TxHistory.record(rid,"approve_preflight_fail",tx_status="skipped",
                        amount=amount,wallet=to_addr,network=cfg["network"],error="invalid amount")
                    fail_cnt+=1; continue
                if amount>SANITY_MAX_AMOUNT:
                    self.log(f"#{rid}: ⛔ Sanity ceiling",tag="fail")
                    TxHistory.record(rid,"approve_blocked_sanity",tx_status="skipped",
                        amount=amount,wallet=to_addr,network=cfg["network"],error="sanity ceiling")
                    fail_cnt+=1; continue
                if not is_valid_address(to_addr):
                    self.log(f"#{rid}: ⛔ Invalid address",tag="fail")
                    TxHistory.record(rid,"approve_preflight_fail",tx_status="skipped",
                        amount=amount,wallet=to_addr,network=cfg["network"],error="invalid address")
                    fail_cnt+=1; continue
                # LAYER 1B
                if AddressBlocklist.is_blocked(to_addr):
                    self.log(f"#{rid}: ⛔ BLOCKLISTED",tag="preflight")
                    TxHistory.record(rid,"approve_blocked_address",tx_status="skipped",
                        amount=amount,wallet=to_addr,network=cfg["network"],error="blocklisted")
                    fail_cnt+=1; continue
                if not simulate:
                    max_tx=float(live_cfg.get("max_tx_amount") or 0)
                    if max_tx>0 and amount>max_tx:
                        self.log(f"#{rid}: ⛔ Per-tx cap {fmt(max_tx)}",tag="preflight")
                        TxHistory.record(rid,"approve_blocked_cap",tx_status="skipped",
                            amount=amount,wallet=to_addr,network=cfg["network"],error="per-tx cap")
                        fail_cnt+=1; continue
                    daily_lim=float(live_cfg.get("daily_limit") or 0)
                    if daily_lim>0:
                        used=DailyLimitTracker.today_total()+batch_total
                        if used+amount>daily_lim:
                            self.log(f"#{rid}: ⛔ Daily limit",tag="preflight")
                            TxHistory.record(rid,"approve_blocked_daily_limit",tx_status="skipped",
                                amount=amount,wallet=to_addr,network=cfg["network"],error="daily limit")
                            fail_cnt+=1; continue
                    rn=int(live_cfg.get("rate_limit_count") or 0)
                    rw=float(live_cfg.get("rate_limit_window") or DEFAULT_RATE_LIMIT_WINDOW)
                    now_ts=time.time(); rate_ts=[t for t in rate_ts if now_ts-t<rw]
                    if rn>0 and len(rate_ts)>=rn:
                        self.log(f"⛔ Rate limit hit — remaining skipped",tag="fail")
                        fail_cnt+=len(recs)-recs.index(rec); break
                try:
                    stored=PendingTxStore.get(rid)
                    if stored:
                        tx_hash=stored["tx_hash"]
                        self.log(f"#{rid}: ♻ Stored TX, retrying API.",tag="stored")
                        if not simulate and chain:
                            try:
                                receipt=chain.wait_for_receipt(tx_hash,timeout=60)
                                chain.validate_receipt(receipt,tx_hash)
                            except TxFailedError as tf:
                                self.log(f"#{rid}: ❌ Stored TX reverted.",tag="fail")
                                TxHistory.record(rid,"approve_failed_tx",tx_hash=tx_hash,
                                    tx_status="reverted",amount=amount,wallet=to_addr,
                                    network=cfg["network"],error=str(tf))
                                PendingTxStore.remove(rid); fail_cnt+=1; continue
                            except ChainError: pass
                    elif simulate:
                        tx_hash=f"SIMULATED-{rid}-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
                        self.log(f"#{rid}: [SIM] would send {fmt(amount)} → {to_addr}",tag="sim")
                    else:
                        self.log(f"#{rid}: 🔍 Pre-flight…",tag="ok")
                        chain.preflight_checks(cfg["from_address"],to_addr,amount,
                                               cfg["token_contract"],cfg["decimals"],required_gas_native=0.002)
                        self.log(f"#{rid}: 📡 Sending {fmt(amount)} → {to_addr[:20]}…",tag="ok")
                        tx_hash=chain.send_token(pk,cfg["from_address"],to_addr,amount,
                                                  cfg["token_contract"],cfg["decimals"],chain_id,nonce)
                        nonce+=1
                        PendingTxStore.put(rid,tx_hash,to_addr,amount,cfg["network"])
                        url=explorer+tx_hash if explorer else ""
                        self.log(f"#{rid}: 📤 {tx_hash[:22]}…",url=url,tag="ok")
                        try:
                            receipt=chain.wait_for_receipt(tx_hash,timeout=TX_WAIT_TIMEOUT)
                        except ChainError as ce:
                            self.log(f"#{rid}: ⚠ Receipt timeout.",tag="warn")
                            TxHistory.record(rid,"approve_timeout",tx_hash=tx_hash,
                                tx_status="timeout",amount=amount,wallet=to_addr,
                                network=cfg["network"],error=str(ce))
                            fail_cnt+=1; continue
                        try:
                            chain.validate_receipt(receipt,tx_hash)
                            confs=chain.current_block()-receipt.get("blockNumber",0)
                            self.log(f"#{rid}: ✅ TX SUCCESS ({confs} confs).",tag="ok")
                        except TxFailedError as tf:
                            self.log(f"#{rid}: ❌❌ TX REVERTED — NOT approving.",tag="fail")
                            TxHistory.record(rid,"approve_failed_tx",tx_hash=tx_hash,
                                tx_status="reverted",amount=amount,wallet=to_addr,
                                network=cfg["network"],error=str(tf))
                            url=explorer+tx_hash if explorer else ""
                            self.ui(lambda w=rid,h=tx_hash,u=url:
                                messagebox.showerror(f"TX FAILED #{w}",
                                    f"TX failed (reverted):\\n{h}\\n\\nCause: {tf.reason}\\n\\nExplorer: {u}"))
                            fail_cnt+=1; continue
                        if confs<TX_CONFIRMATIONS_REQ: time.sleep(6)
                    tx_status="simulated" if simulate else "success"
                    self.api.approve(rid,tx_hash,
                        note=(f"{'SIMULATED' if simulate else cfg['network']}|"
                              f"{amount_field}={fmt(amount)}"))
                    PendingTxStore.remove(rid)
                    TxHistory.record(rid,"approve",tx_hash=tx_hash,tx_status=tx_status,
                        amount=amount,wallet=to_addr,network=cfg["network"])
                    if not simulate:
                        DailyLimitTracker.add(amount); rate_ts.append(time.time()); batch_total+=amount
                    url=(explorer+tx_hash) if (explorer and not simulate) else ""
                    self.log(f"#{rid}: ✅ APPROVED {fmt(amount)}.",url=url,tag="ok")
                    ok_cnt+=1
                except PreflightError as pe:
                    self.log(f"#{rid}: ⛔ Pre-flight: {pe}",tag="preflight")
                    TxHistory.record(rid,"approve_preflight_fail",tx_status="skipped",
                        amount=amount,wallet=to_addr,network=cfg["network"],error=str(pe))
                    fail_cnt+=1
                except ApiError as ae:
                    self.log(f"#{rid}: ⚠ API approve failed: {ae}",tag="warn")
                    TxHistory.record(rid,"approve_api_fail",tx_status="success_tx_api_fail",
                        amount=amount,wallet=to_addr,network=cfg["network"],error=str(ae))
                except Exception as exc:
                    self.log(f"#{rid}: ❌ ERROR: {exc}",tag="fail")
                    TxHistory.record(rid,"approve_error",tx_status="error",
                        amount=amount,wallet=to_addr,network=cfg["network"],error=str(exc))
                    fail_cnt+=1
            self.log(f"Batch done: {ok_cnt} ok, {fail_cnt} failed.",
                     tag="ok" if fail_cnt==0 else "warn")
            self._approval_timestamps = rate_ts
            return ok_cnt, fail_cnt

        def done(result):
            if result is None: return
            s,f=result; self.refresh_pending(); self.refresh_all(); self.refresh_history()
            if f==0: messagebox.showinfo("Done",f"✅ {s} approved.")
            else: messagebox.showwarning("Done with errors",f"⚠ {s} ok, {f} failed.")
    ...
    def _build_balances_tab(self):
        p=self.tab_balances
        ac=tk.Frame(p,bg=C["card"],highlightbackground=C["border"],highlightthickness=1)
        ac.pack(fill="x",padx=10,pady=(8,4))
        ai=tk.Frame(ac,bg=C["card"],padx=12,pady=8); ai.pack(fill="x")
        tk.Label(ai,text="Hot Wallet:",font=("Segoe UI",9,"bold"),bg=C["card"],fg=C["text_dim"]).pack(side="left")
        self.bal_addr_lbl=tk.Label(ai,text=self.cfg.get("from_address") or "(not set)",
        ...
        self.bal_next_lbl=tk.Label(bar,text="",font=("Segoe UI",8),bg=C["bg"],fg=C["text_dim"])
        ...
        self.bal_canvas.pack(side="left",fill="both",expand=True); vsb.pack(side="right",fill="y")
        ...
        for c,w in (("symbol",70),("contract",390),("decimals",70),("network",130)):
            self.watch_tree.heading(c,text=c.title()); self.watch_tree.column(c,width=w,anchor="w")
    ...
    def _add_watch_token(self):
        win=tk.Toplevel(self.root); win.title("Add Custom Token")
        ...
        def _confirm():
            ...
            if not is_valid_address(addr):
                messagebox.showwarning("Invalid","Not a valid 0x… address.",parent=win); return
            win.destroy()
            ...
            def done(res):
                sym,dec=res
                sym=simpledialog.askstring("Symbol",f"Symbol:",initialvalue=sym,parent=self.root) or sym
                try:
                    dec=int(simpledialog.askstring("Decimals",f"Decimals:",
    ...
    def _remove_watch_token(self):
        ...
        tokens=self.cfg.get("extra_tokens",[])
        if 0<=idx<len(tokens):
            tokens.pop(idx); self.cfg["extra_tokens"]=tokens
    ...
    def refresh_balances(self, silent=False):
        ...
        if not wallet:
            ...
            return
        self._bal_countdown=self._interval("balances",LIVE_INTERVAL_BALANCES,BACKGROUND_INTERVAL_BALANCES)
        extras=list(self.cfg.get("extra_tokens",[])); self.bal_status.config(text="⏳ Scanning…")
        def _scan(net_key,net_cfg):
            ...
            for spec in [{"symbol":net_cfg["native"],"address":"native","type":"native","decimals":18}]+\
                        net_cfg.get("tokens",[])+[t for t in extras if t.get("network")==net_key]:
                sym=spec.get("symbol","?"); addr=spec.get("address","native")
                tt=spec.get("type","known") if "type" in spec else "extra"
                try:
                    bal=(chain.native_balance(wallet) if addr=="native"
                         else chain.token_balance(addr,wallet,spec.get("decimals",18)))
                    items.append({"symbol":sym,"balance":bal,"contract":addr,
                                   "decimals":spec.get("decimals",18),"type":tt,"network":net_key})
                except Exception as e:
                    items.append({"symbol":sym,"balance":None,"error":str(e),
                                   "contract":addr,"type":tt,"network":net_key})
        ...
        def done(results):
            ...
            self.bal_status.config(
                text=f"✓ Scanned • {nonzero} with balance • {datetime.now().strftime('%H:%M:%S')}")
    ...
    def _render_all_network_cards(self, results: dict):
        ...
        for net_key,items in results.items():
            ...
            for i,item in enumerate(visible):
                ...
                has_bal=item.get("balance") is not None; is_zero=has_bal and float(item.get("balance",0))==0
    ...
    def _build_security_tab(self):
        inner=make_scrollable(self.tab_security); inner.columnconfigure(0,weight=1)
        pad={"padx":14,"pady":6}
        ...
        def section(title,row):
            f=ttk.LabelFrame(inner,text=title,padding=12); f.grid(row=row,column=0,sticky="ew",**pad)
            f.columnconfigure(1,weight=1); return f

        ks_box=section("🛑 Emergency Kill Switch",1)
        tk.Label(ks_box,text="Instantly blocks ALL live on-chain approvals (manual and auto-approve).",
                 bg=C["bg"],fg=C["text_dim"],wraplength=680,justify="left").grid(row=0,column=0,columnspan=2,sticky="w")
        ...
        lim_box=section("📊 Transaction Limits (LIVE mode only)",2)
        ...
        ttk.Label(lim_box,text="Rate limit (count / window s)").grid(row=3,column=0,sticky="w",pady=3)
        ...
        self.var_block_addr=tk.StringVar(); self.var_block_reason=tk.StringVar()
        ...
        tk.Label(st_box,
                 text=f"Key auto-locks after {IDLE_LOCK_SECONDS//60}min idle (also stops auto-approve). "
                      f"{MAX_PASSPHRASE_ATTEMPTS} failed attempts → {LOCKOUT_DURATION_SECONDS//60}min lockout.",
                 bg=C["bg"],fg=C["text_dim"],wraplength=680,justify="left").grid(row=0,column=0,columnspan=2,sticky="w")
        ...
        ttk.Button(aud_box,text="🔍 Verify Audit Log Integrity",
                   style="Accent.TButton",command=self._verify_audit_log).grid(row=0,column=0,sticky="w",pady=6)

        # Auto-Approve log viewer
        al_box=section("🤖 Auto-Approve Session Log",6)
        ttk.Button(al_box,text="📂 Open auto_approve_log.json",
                   command=lambda: (webbrowser.open(AUTO_APPROVE_LOG_PATH)
                                    if os.path.exists(AUTO_APPROVE_LOG_PATH)
                                    else messagebox.showinfo("Not found","No auto-approve log yet."))
                   ).grid(row=0,column=0,sticky="w",pady=6)
        self.auto_log_count_lbl=ttk.Label(al_box,text="",foreground=C["text_dim"])
        self.auto_log_count_lbl.grid(row=0,column=1,sticky="w",padx=8)

        sl_box=section("📋 Security Event Log",7)
    ...
    def _activate_kill_switch(self):
        if not messagebox.askyesno("Confirm Kill Switch",
                "Block ALL live approvals (manual + auto) immediately?",icon="warning"): return
        reason=simpledialog.askstring("Reason","Reason (optional):",parent=self.root) or ""
        self.cfg["kill_switch_enabled"]=True; ConfigStore.save(self.cfg)
        SecurityLog.record("kill_switch_on",reason)
        if self.auto_engine.is_running:
            self.auto_engine.stop()
            self.log("🛑 Kill switch activated — auto-approve engine stopped.",tag="fail")
        else:
            self.log("🛑 KILL SWITCH ACTIVATED.",tag="fail")
        self._refresh_security_banner()
        messagebox.showinfo("Kill Switch Active","🛑 All live approvals blocked.")

    def _deactivate_kill_switch(self):
        if not messagebox.askyesno("Confirm","Resume live approvals?"): return
        self.cfg["kill_switch_enabled"]=False; ConfigStore.save(self.cfg)
        SecurityLog.record("kill_switch_off","")
        self.log("✅ Kill switch deactivated.",tag="ok")
        self._refresh_security_banner()
    ...
    def _save_security_limits(self):
        ...
        except ValueError: messagebox.showerror("Invalid","Limits must be numeric."); return
    ...
    def _add_blocked_address(self):
        ...
        if not is_valid_address(addr):
            messagebox.showwarning("Invalid","Enter a valid 0x… address."); return
    ...
    def _remove_blocked_address(self):
        ...
        AddressBlocklist.remove(addr); SecurityLog.record("address_unblocked",addr)
        self._reload_block_tree()
    ...
    def _manual_lock(self):
        ...
        if self.auto_engine.is_running:
            self.auto_engine.stop()
            self.log("🔒 Manual lock — auto-approve engine stopped.",tag="warn")
        SecurityLog.record("manual_lock","")
    ...
    def _verify_audit_log(self):
        ...
        if ok:
            messagebox.showinfo("Integrity OK",
                f"✅ Audit log verified. {legacy} legacy records skipped.")
        else:
            messagebox.showerror("TAMPERING DETECTED",
                f"❌ Integrity FAILED at record #{broken_at}. Investigate immediately.")
        SecurityLog.record("integrity_check",f"ok={ok} broken_at={broken_at}")
    ...
    def _reload_security_log(self):
        ...
        if hasattr(self,"auto_log_count_lbl"):
            cnt=len(AutoApproveLog.all())
            self.auto_log_count_lbl.config(text=f"{cnt} auto-approve log entries")

    def _refresh_security_banner(self):
        if not hasattr(self,"kill_banner"): return
        if self.cfg.get("kill_switch_enabled"):
            self.kill_banner.config(text="🛑 KILL SWITCH ACTIVE — all live approvals blocked",
                                     bg=C["live_bg"],fg=C["live_fg"])
        else:
            self.kill_banner.config(text="✅ Normal operation — live approvals allowed",
                                     bg=C["row_appr"],fg=C["success"])
        locked,until=LockoutGuard.is_locked()
        if locked:
            wait=max(int(until-datetime.now().timestamp()),0)
            self.lockout_status_lbl.config(text=f"🔒 Locked — retry in {wait}s",foreground=C["danger"])
        else:
            self.lockout_status_lbl.config(text="🔓 No active lockout.",foreground=C["success"])
        if self.runtime_pk:
            idle=int(time.time()-self._last_activity)
            self.idle_status_lbl.config(
                text=f"🔑 Key in memory — auto-locks in {max(IDLE_LOCK_SECONDS-idle,0)}s (idle {idle}s)")
        else:
            self.idle_status_lbl.config(text="🔒 Key not loaded in memory.")
        if hasattr(self,"daily_usage_lbl"):
            used=DailyLimitTracker.today_total(); cap=float(self.cfg.get("daily_limit") or 0)
            pct=f" ({used/cap*100:.0f}%)" if cap>0 else ""
            self.daily_usage_lbl.config(
                text=f"Today's approved: {fmt(used)} / {fmt(cap) if cap>0 else '∞'}{pct}")
    ...
    def _build_history_tab(self):
        ...
        ttk.Button(bar,text="📂 Open File",
                   command=lambda: webbrowser.open(TX_HISTORY_PATH)).pack(side="left",padx=4)
        ...
        hf=ttk.Combobox(bar,textvariable=self.var_hist_filter,state="readonly",width=26,
        ...
        self.hist_next_lbl=tk.Label(bar,text="",font=("Segoe UI",8),bg=C["bg"],fg=C["text_dim"])
        ...
        hist_widths=(40,60,170,90,90,200,100,155,200)
        ...
        for col,head,w in zip(hist_cols,hist_heads,hist_widths):
            self.hist_tree.heading(col,text=head); self.hist_tree.column(col,width=w,minwidth=40,anchor="w")
        ...
        vhsb.grid(row=0,column=1,sticky="ns"); hhsb.grid(row=1,column=0,sticky="ew")
        wrap.rowconfigure(0,weight=1); wrap.columnconfigure(0,weight=1)
    ...
    def refresh_history(self):
        ...
        data=TxHistory.all()
        flt=getattr(self,"var_hist_filter",None); fval=flt.get() if flt else "all"
        if fval!="all": data=[r for r in data if r.get("action")==fval]
        data=list(reversed(data))
        for row in self.hist_tree.get_children(): self.hist_tree.delete(row)
        for r in data:
            wallet=r.get("wallet","")
            sw=(wallet[:12]+"…"+wallet[-6:]) if len(wallet)>20 else wallet
            tag=r.get("tx_status","")
            self.hist_tree.insert("","end",values=(
                r.get("id",""),r.get("withdrawal_id",""),r.get("action",""),r.get("tx_status",""),
                fmt(r.get("amount",0)),sw,r.get("network",""),(r.get("timestamp") or "")[:19],
                (r.get("error") or "")[:80]),tags=(tag,))
    ...
    def _build_settings_tab(self):
        inner=make_scrollable(self.tab_settings); inner.columnconfigure(0,weight=1)
        pad={"padx":14,"pady":6}
        def section(title,row):
            f=ttk.LabelFrame(inner,text=title,padding=12); f.grid(row=row,column=0,sticky="ew",**pad)
            f.columnconfigure(1,weight=1); return f
        ...
        ttk.Button(api_box,text="Test API",command=self._test_api,
                   style="Accent.TButton").grid(row=2,column=1,sticky="w",pady=6,padx=(8,0))

        chain_box=section("⛓  Blockchain",1)
        ...
        self.net_lbl=ttk.Label(chain_box,text=NETWORK_PRESETS.get(self.cfg["network"],{}).get("label",""),
                                foreground=C["accent"])
        ...
        for (lbl,attr,row) in [("RPC URL","var_rpc",1),("Token Contract","var_contract",2)]:
            ...
            setattr(self,attr,tk.StringVar(value=self.cfg["rpc_url" if "rpc" in attr else "token_contract"]))
            ttk.Entry(chain_box,textvariable=getattr(self,attr)).grid(
                row=row,column=1,columnspan=2,sticky="ew",padx=(8,0),pady=3)
        ttk.Label(chain_box,text="Decimals").grid(row=3,column=0,sticky="w",pady=3)
        ...
        ttk.Button(chain_box,text="Test RPC",command=self._test_rpc,
                   style="Accent.TButton").grid(row=5,column=1,sticky="w",pady=6,padx=(8,0))

        wallet_box=section("💳  Sending Wallet",2)
        tk.Label(wallet_box,text="⚠  This wallet pays customers. Only fund what you need.",
        ...
        ttk.Checkbutton(pk_f,text="show",variable=self.var_show_pk,
                         command=lambda: self.pk_entry.config(show="" if self.var_show_pk.get() else "*")).pack(side="left",padx=4)
        ...
        ttk.Button(btn_f,text="Save Wallet",command=self._save_wallet,style="Success.TButton").pack(side="left",padx=3)
        ttk.Button(btn_f,text="Clear Key",  command=self._clear_pk).pack(side="left",padx=3)
        ttk.Button(btn_f,text="Check Balances",command=self._check_balances,style="Accent.TButton").pack(side="left",padx=3)

        sim_box=section("⚠  LIVE / SIMULATION MODE",3); sim_box.columnconfigure(0,weight=1)
        self.sim_banner=tk.Label(sim_box,text="",font=("Segoe UI",10,"bold"),anchor="center",pady=10)
        self.sim_banner.grid(row=0,column=0,columnspan=3,sticky="ew")
        def _upd(*_):
            if self.var_simulate.get():
                self.sim_banner.config(text="🟡  SIMULATION MODE",bg=C["sim_bg"],fg=C["sim_fg"])
            else:
                self.sim_banner.config(text="🔴  LIVE MODE — real funds WILL be sent",
                                        bg=C["live_bg"],fg=C["live_fg"])
            self._update_mode_badge()
        self.var_simulate.trace_add("write",_upd); _upd()
        bf=ttk.Frame(sim_box); bf.grid(row=1,column=0,pady=(8,0))
        ttk.Button(bf,text="Enable SIMULATION",command=lambda: self.var_simulate.set(True),
                   style="Accent.TButton").pack(side="left",padx=6)
        ttk.Button(bf,text="Enable LIVE MODE",command=self._go_live,
                   style="Danger.TButton").pack(side="left",padx=6)

        lr_box=section("🔄  Auto-Refresh",4)
        self.var_pause=tk.BooleanVar(value=False)
        ttk.Checkbutton(lr_box,text="Pause all auto-refresh",variable=self.var_pause,
                         command=lambda: setattr(self,"_live_paused",self.var_pause.get())).grid(row=0,column=0,sticky="w")

        path_box=section("📁  Data File Locations",5)
        for i,(lbl,path) in enumerate([
            ("TX History",TX_HISTORY_PATH),("Auto-Approve Log",AUTO_APPROVE_LOG_PATH),
            ("Pending TX Store",PENDING_TX_PATH),("Daily Limits",DAILY_LIMIT_PATH),
            ("Address Blocklist",BLOCKLIST_PATH),("Security Log",SECURITY_LOG_PATH),
            ("Config",CONFIG_PATH),
        ]):
            tk.Label(path_box,text=lbl,font=("Segoe UI",9,"bold"),
                     bg=C["bg"],fg=C["text_dim"],width=18,anchor="w").grid(row=i,column=0,sticky="w",pady=2)
            ...
            tk.Button(path_box,text="Open",font=("Segoe UI",8),bg=C["bg"],fg=C["accent"],
                      relief="flat",bd=0,cursor="hand2",command=_open).grid(row=i,column=2,padx=4)

        bot=ttk.Frame(inner); bot.grid(row=6,column=0,sticky="ew",padx=14,pady=8)
    ...
    def _clear_pk(self):
        ...
        messagebox.showinfo("Cleared","Key removed.")
    ...
    def _test_api(self):
        ...
        def done(st): messagebox.showinfo("API OK",f"Pending: {st.get('pending_count',0)} Total: {st.get('total_requests',0)}")
    ...
    def _test_rpc(self):
        ...
        def done(cid): messagebox.showinfo("RPC OK",f"Chain ID: {cid}")
        def err(e): messagebox.showerror("RPC Failed",str(e))
        self.run_bg(work,on_done=done,on_error=err)

    def _check_balances(self):
        self._collect()
        if not self.cfg["from_address"]: messagebox.showwarning("Missing","Enter From Address."); return
        self.nb.select(self.tab_balances); self.refresh_balances()

    def _go_live(self):
        if self.cfg.get("kill_switch_enabled"):
            messagebox.showwarning("Kill Switch Active","Deactivate the kill switch first.")
        if not messagebox.askyesno("Enable LIVE mode?",
            "⚠ Real on-chain transactions will be sent. Continue?",icon="warning"): return
        if not messagebox.askyesno("Second confirmation",
